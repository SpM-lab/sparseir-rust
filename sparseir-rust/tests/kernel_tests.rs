...
use sparseir_rust::{LogisticKernel, SymmetryType, CustomNumeric, compute_logistic_kernel, CentrosymmKernel};
use twofloat::TwoFloat;
use dashu_float::{DBig, round::mode::HalfAway, Context};
use dashu_base::{Approximation, Abs};
use std::str::FromStr;
...
fn test_logistic_kernel_compute_precision<T: CustomNumeric>(lambda: f64, x: f64, y: f64, tolerance: f64) {
    ...
    assert!(
        ...
        lambda, x, y, std::any::type_name::<T>(), result_t.to_f64(), extract_f64(result_dbig.to_f64()), error_t_dbig_f64, tolerance
    );
}
    ...
    for lambda in lambdas {
        for x in [-1.0, 0.0, 1.0] {
            for y in [-1.0, 0.0, 1.0] {
                ...
                test_logistic_kernel_compute_precision::<TwoFloat>(lambda, x, y, TOLERANCE_TWOFLOAT);
            }
        }
    }
}
...
fn compute_reduced_dbig(lambda: DBig, x: DBig, y: DBig, symmetry: SymmetryType, ctx: &Context<HalfAway>) -> DBig {
    match symmetry {
        SymmetryType::Even => {
            // K(x, y) + K(x, -y)
...
fn test_logistic_kernel_compute_reduced_precision<T: CustomNumeric>(
    lambda: f64, x: f64, y: f64, symmetry: SymmetryType, tolerance: f64
) {
    // Convert inputs to type T
    let x_t: T = T::from_f64(x);
    let y_t: T = T::from_f64(y);
...
fn test_logistic_kernel_compute_reduced_different_lambdas() {
    ...
    for lambda in lambdas {
        for x in [0.0, 0.5, 1.0] {  // x >= 0
            for y in [0.0, 0.5, 1.0] {  // y >= 0
                for symmetry in [SymmetryType::Even, SymmetryType::Odd] {
                    // Test both f64 and TwoFloat implementations
                    test_logistic_kernel_compute_reduced_precision::<f64>(lambda, x, y, symmetry, TOLERANCE_F64);
                    test_logistic_kernel_compute_reduced_precision::<TwoFloat>(lambda, x, y, symmetry, TOLERANCE_TWOFLOAT);
                }
            }
        }
    }
...
fn test_compute_reduced_negative_y_works() {
    ...
    assert!(result.is_finite(), "compute_reduced should work for negative y");
}
...
fn test_compute_reduced_positive_y_works() {
    ...
    assert!(result.is_finite(), "compute_reduced should work for positive y");
}
...
fn test_compute_reduced_symmetry_types() {
    ...
    assert_ne!(result_even, result_odd, "Even and odd results should be different");
}
