...
fn test_interpolate_2d_quadratic_polynomial() {
    ...
    let gauss_x = legendre_generic::<TwoFloat>(4).reseat(TwoFloat::from_f64(-1.0), TwoFloat::from_f64(1.0));
    ...
    for i in 0..4 {
        for j in 0..4 {
            ...
            let interpolated = evaluate_2d_legendre_polynomial(
                x, y, &coeffs, &gauss_x, &gauss_y
            );
            assert!((interpolated - expected).abs() < TwoFloat::from_f64(1e-12),
                "Interpolation failed at ({}, {}): expected {}, got {}",
                x, y, expected, interpolated);
        }
    }
    ...
    for (x, y) in test_points {
        ...
        assert!(error < TwoFloat::from_f64(1e-14),
            "Interpolation failed at ({}, {}): expected {}, got {}, error = {}",
            x, y, expected, interpolated, error);
    }
}
...
fn test_interpolate2d_struct_generic<T: CustomNumeric + 'static>() {
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            assert!(error < T::from_f64(1e-12), 
                    "Interpolation error at Gauss point ({}, {}): {} > 1e-12", i, j, error);
        }
    }
