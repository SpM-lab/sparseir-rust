...
fn test_get_tnl_basic_values() {
    ...
    println!("get_tnl(0, 1.0) = {}, expected = {}", result_0_1, expected_0_1);
    ...
    println!("get_tnl(0, π) = {}, expected = {}", result_0_pi, expected_0_pi);
    ...
    println!("get_tnl(1, 1.0) = {}, expected = {}", result_1_1, expected_1_1);
    
    // Test negative w (should apply conjugation)
    let result_0_neg1 = PiecewiseLegendreFT::<Fermionic>::get_tnl(0, -1.0);
    println!("get_tnl(0, -1.0) = {}, should be conjugate of positive", result_0_neg1);
...
fn test_constant_polynomial_fourier_transform() {
    ...
    for n in 0..5 {
        if let Ok(omega) = sparseir_rust::freq::MatsubaraFreq::new(n) {
            ...
            if n == 0 {
                assert!(result.norm() > 0.1, "Constant polynomial should be non-zero at n=0");
            } else {
                // For n > 0, the result should be very close to zero
                println!("  Norm at n={}: {}", n, result.norm());
                if result.norm() > 1e-6 {
                    println!("  WARNING: Constant polynomial has significant value at n={}", n);
                }
            }
        }
    }
