    ...
    match n {
        ...
        _ => {
            ...
            for k in 2..=n {
                ...
                let p2 = ((TwoFloat::from(2.0) * k1_f + TwoFloat::from(1.0)) * x * p1 - k1_f * p0) / k_f;
                p0 = p1;
                p1 = p2;
            }
            p1
...
fn test_high_precision_legendre_f64() {
    ...
    let x_expected = Array1::from(vec![
        -0.9894009349916499, -0.9445750230732325,
    ...
    let w_expected = Array1::from(vec![
        0.027152459411754124, 0.06225352393864806,
    ...
    // Check x values
    for i in 0..n {
        assert!((rule.x[i] - x_expected[i]).abs() < tolerance, 
                "x[{}] mismatch: expected {}, got {}", i, x_expected[i], rule.x[i]);
    }
    
    // Check w values
    for i in 0..n {
        assert!((rule.w[i] - w_expected[i]).abs() < tolerance,
                "w[{}] mismatch: expected {}, got {}", i, w_expected[i], rule.w[i]);
    }
    ...
    for i in 0..rule.x.len() {
        ...
        assert!((rule.x_forward[i] - expected_forward).abs() < tolerance,
                "x_forward[{}] inconsistent", i);
        assert!((rule.x_backward[i] - expected_backward).abs() < tolerance,
                "x_backward[{}] inconsistent", i);
    }
}
...
fn test_high_precision_legendre_twofloat() {
    ...
    assert!((weight_sum - TwoFloat::from(2.0)).abs() < TwoFloat::from(1e-14),
            "Sum of weights should be 2.0, got {}", weight_sum);
}
...
fn test_legendre_polynomial_at_nodes() {
    ...
    for i in 0..n {
        ...
        assert!((p1 - rule.x[i]).abs() < 1e-14, "P_1(x[{}]) should equal x[{}]", i, i);
    }
    ...
    for i in 0..n {
        ...
        assert!(pn.abs() < 1e-12, "P_{}(x[{}]) should be approximately 0, got {}", n, i, pn);
    }
}
...
fn test_legendre_polynomial_twofloat_at_nodes() {
    ...
    for i in 0..n {
        ...
        assert!((p0 - TwoFloat::from(1.0)).abs() < TwoFloat::from(1e-15), 
                "P_0(x[{}]) should be 1.0", i);
    }
    
    // Test that P_1(x) = x at all nodes
    for i in 0..n {
        let p1 = legendre_polynomial_twofloat(1, rule.x[i]);
        assert!((p1 - rule.x[i]).abs() < TwoFloat::from(1e-15), 
                "P_1(x[{}]) should equal x[{}]", i, i);
    }
    
    // Test that P_n(x) = 0 at all nodes
    for i in 0..n {
        let pn = legendre_polynomial_twofloat(n, rule.x[i]);
        assert!(pn.abs() < TwoFloat::from(1e-14), 
                "P_{}(x[{}]) should be approximately 0, got {}", n, i, pn);
    }
}
...
fn test_large_legendre_rule_high_precision() {
    ...
    assert!((weight_sum - 2.0).abs() < 1e-14, 
            "Sum of weights should be 2.0, got {}", weight_sum);
...
fn test_piecewise_high_precision() {
    ...
    let weight_sum: f64 = rule.w.iter().sum();
    assert!((weight_sum - 7.0).abs() < 1e-13, 
            "Sum of weights should be 7.0, got {}", weight_sum);
}
