...
fn test_rescale_domain_vector() {
    ...
    let data1 = mdarray::DTensor::<f64, 2>::from_fn([2, 1], |idx| if idx[0] == 0 { 1.0 } else { 0.5 });
    let data2 = mdarray::DTensor::<f64, 2>::from_fn([2, 1], |idx| if idx[0] == 0 { 2.0 } else { 1.0 });
...
fn test_rescale_with_symmetry_change() {
    ...
    let data = mdarray::DTensor::<f64, 2>::from_fn([2, 1], |idx| if idx[0] == 0 { 1.0 } else { 0.5 });
    let knots = vec![-1.0, 1.0];
