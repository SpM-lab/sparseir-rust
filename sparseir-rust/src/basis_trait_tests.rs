...
fn test_basis_trait_generic() {
    ...
    let basis_f = FiniteTempBasis::<LogisticKernel, Fermionic>::new(kernel_f, beta, Some(epsilon), None);
    check_basis(&basis_f);
    
    let kernel_b = LogisticKernel::new(beta * wmax);
    let basis_b = FiniteTempBasis::<LogisticKernel, Bosonic>::new(kernel_b, beta, Some(epsilon), None);
    check_basis(&basis_b);
}
...
fn test_basis_trait_evaluate_tau() {
    ...
    for i in 0..tau_points.len() {
        for l in 0..basis.size() {
            assert!(matrix[[i, l]].is_finite(), "matrix[{}, {}] = {}", i, l, matrix[[i, l]]);
        }
    }
}
