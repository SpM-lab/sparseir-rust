...
pub trait CentrosymmKernel: Send + Sync {
    ...
    fn compute_reduced(&self, x: TwoFloat, y: TwoFloat, sign: i32) -> TwoFloat;

    fn compute_reduced_f64(&self, x: f64, y: f64, sign: i32) -> f64;
...
impl CentrosymmKernel for LogisticKernel {
    ...
    // TODO: implement faster algorithm in f64
    fn compute_f64(&self, x: f64, y: f64) -> f64 {
        return self.compute(TwoFloat::from_f64(x), TwoFloat::from_f64(y)).into();
    }

    fn compute_reduced(&self, x: TwoFloat, y: TwoFloat, sign: i32) -> TwoFloat {
        return self.compute(x, y) + TwoFloat::from(sign) * self.compute(x, -y);
    }

    fn compute_reduced_f64(&self, x: f64, y: f64, sign: i32) -> f64 {
        let sign_f = sign.to_f64().expect("sign must be +/- 1!");
        return self.compute_f64(x, y) + sign_f * self.compute_f64(x, -y);
    }
