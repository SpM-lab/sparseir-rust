//! Parameterized RRQR tests for f64 and Df64 precision
...
fn to_f64_matrix(matrix: &Tensor<Df64Precision, (usize, usize)>) -> Tensor<f64, (usize, usize)> {
    let (m, n) = *matrix.shape();
    Tensor::from_fn((m, n), |idx| matrix[[idx[0], idx[1]]].to_f64())
}
    ...
    T: Copy + std::ops::Sub<Output = T>,
{
    ...
    Tensor::from_fn(shape_a, |idx| a[[idx[0], idx[1]]] - b[[idx[0], idx[1]]])
}
...
fn test_rrqr_simple_matrix_template_f64() {
    let mut a = Tensor::from_fn((2, 2), |idx| match (idx[0], idx[1]) {
        (0, 0) => 2.0,
        (0, 1) => 1.0,
        (1, 0) => 4.0,
        (1, 1) => 3.0,
    ...
    let a_original = Tensor::from_fn((2, 2), |idx| match (idx[0], idx[1]) {
        (0, 0) => 2.0,
        (0, 1) => 1.0,
        (1, 0) => 4.0,
        (1, 1) => 3.0,
    ...
    assert!(
        max_error < 100.0 * f64::EPSILON,
        "Reconstruction error too large: {}",
        max_error
    );
}

/// Template function for RRQR simple matrix test (Df64 version)
fn test_rrqr_simple_matrix_template_df64() {
    let matrix_f64 = Tensor::from_fn((2, 2), |idx| match (idx[0], idx[1]) {
        (0, 0) => 2.0,
        (0, 1) => 1.0,
        (1, 0) => 4.0,
        (1, 1) => 3.0,
    ...
    let mut a = to_df64_matrix(&matrix_f64);
    let rtol = Df64Precision::epsilon();
...
fn test_rrqr_orthogonal_q_template_f64() {
    ...
    assert!(
        max_error < 1e-10,
        "Q is not orthogonal, max error: {}",
        max_error
    );
}

/// Template function for RRQR orthogonal Q test (Df64 version)
fn test_rrqr_orthogonal_q_template_df64() {
    let matrix_f64 = Tensor::from_fn((2, 2), |idx| match (idx[0], idx[1]) {
        (0, 0) => 3.0,
        (0, 1) => 4.0,
        (1, 0) => 4.0,
...
#[test]
fn test_rrqr_identity_matrix_df64() {
    test_rrqr_identity_matrix_template::<Df64Precision>();
}

#[test]
fn test_rrqr_simple_matrix_df64() {
    test_rrqr_simple_matrix_template_df64();
}

#[test]
fn test_rrqr_rank_deficient_df64() {
    test_rrqr_rank_deficient_template::<Df64Precision>();
}

#[test]
fn test_rrqr_zero_matrix_df64() {
    test_rrqr_zero_matrix_template::<Df64Precision>();
}

#[test]
fn test_rrqr_single_element_df64() {
    test_rrqr_single_element_template::<Df64Precision>();
}

#[test]
fn test_rrqr_orthogonal_q_df64() {
    test_rrqr_orthogonal_q_template_df64();
}
...
fn test_rrqr_precision_comparison() {
    let matrix_f64 = Tensor::from_fn((3, 3), |idx| match (idx[0], idx[1]) {
    ...
    // Test with Df64
    let mut matrix_tf = to_df64_matrix(&matrix_f64);
    let rtol_tf = Df64Precision::from_f64(f64::EPSILON);
    ...
    for i in 0..m {
        for j in 0..n {
            ...
            // Df64 should be more accurate, so difference should be small
            assert!(
                diff < 100.0 * f64::EPSILON,
                "Q matrix difference at [{}, {}]: {}",
                i,
                j,
                diff
            );
