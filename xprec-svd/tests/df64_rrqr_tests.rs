...
#[test]
fn test_df64_rrqr_identity() {
    let matrix_f64: Tensor<f64, (usize, usize)> = eye(3);
    let mut matrix = to_df64_matrix(&matrix_f64);
    ...
    for i in 0..3 {
        for j in 0..3 {
            ...
            assert!(
                ...
                "Q^T * Q should be identity, got diff = {}",
                diff
            );
        }
    }
...
fn test_df64_rrqr_rank_one() {
    ...
    let mut reconstructed = Tensor::from_elem((3, 3), Df64Precision::from_f64(0.0));
    for i in 0..3 {
        for j in 0..3 {
            for k in 0..rank {
    ...
    // Apply permutation
    let mut permuted = Tensor::from_elem((3, 3), Df64Precision::from_f64(0.0));
    for i in 0..3 {
        for j in 0..3 {
    ...
    let original_f64 = to_f64_matrix(&to_df64_matrix(&matrix_f64));
...
#[test]
fn test_df64_rrqr_precision_comparison() {
...
#[test]
fn test_df64_rrqr_eps_precision() {
    ...
    let mut matrix = to_df64_matrix(&matrix_f64);
    let rtol = Df64Precision::epsilon(); // EPSILON tolerance
    ...
    for i in 0..3 {
        for j in 0..3 {
            let diff = (qtq_f64[[i, j]] - identity[[i, j]]).abs();
            // Df64 should achieve better than machine epsilon precision
            assert!(
                diff < 1e-15,
                "Q^T * Q orthogonality error at [{}, {}]: {}",
                i,
                j,
                diff
            );
