...
fn apply_rotation_left<T: Precision>(
    m: &mut [[T; 2]; 2],
    p: usize,
    q: usize,
    rot: &JacobiRotation<T>,
) {
...
fn apply_rotation_right<T: Precision>(
    m: &mut [[T; 2]; 2],
    p: usize,
    q: usize,
    rot: &JacobiRotation<T>,
...
pub fn jacobi_svd<T: Precision>(matrix: &Tensor<T, (usize, usize)>) -> SVDResult<T> {
    let shape = *matrix.shape();
    let (m, n) = shape;
    let k = m.min(n);
    ...
    for iter in 0..max_iter {
        ...
        for p in 1..diagsize {
            for q in 0..p {
                ...
                let (left_rot, right_rot, (_s1, _s2)) =
                    real_2x2_jacobi_svd(a[[p, p]], a[[p, q]], a[[q, p]], a[[q, q]]);
        ...
        if iter == max_iter - 1 {
            eprintln!(
                "[Jacobi SVD] WARNING: Max iterations ({}) reached without convergence!",
                max_iter
            );
        }
    }
